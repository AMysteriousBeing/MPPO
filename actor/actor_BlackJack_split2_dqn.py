...
import numpy as np
import random
from environment import BlackJackEnv, BlackJackEnv2, BlackJackEnvWrapper
from agent import BlackJackDiscretized2, BlackJackTabular, BlackJackBasic
from model import BlackJackNet5b, BlackJackNet
...
class ActorBlackJackSplit2DQN(Process):
    """
    ...
    def run(self):
        ...
        game_counter = 0
        model = BlackJackNet() if self.debug else BlackJackNet5b()
        model.eval()
        ...
        while not self.actor_routine_status_list[1]:
            ...
            if not self.self_play or self.guide_trajectory:
                ...
                config = {
                    "agent_type": BlackJackBasic if self.debug else BlackJackTabular,
                    "higher_ace_chance": 0.25,
                }
            ...
            while not done:
                ...
                agent_data["state"].append(obs)
                obs2tensor = obs.unsqueeze(0)
                if np.random.random() < self.epsilon:
                    random_q = torch.rand((1, 2))
                    action = torch.argmax(random_q, dim=1).item()
                else:

                    with torch.no_grad():
                        logits = model(obs2tensor)
            ...
            for agent_name, agent_data in episode_data.items():
                ...
                if len(agent_data["state"]) > 0:
                    obs = np.stack(agent_data["state"])
                    ...
                    next_obs = np.array(
                        agent_data["state"][1:] + [agent_data["state"][0]],
                        dtype=np.float32,
                    )
