...
"""
This model is designed for MahJong Feature Agent Adapted2
"""
...
class MahJongCNNNet6_LargeV4(nn.Module):

    def __init__(self, device="cpu"):
        super(MahJongCNNNet6_LargeV4, self).__init__()
        self.device = device
        self.public_p = nn.Sequential(
            nn.Conv2d(139, 256, 3, 1, padding=(1, 0)),  # 128*4,7
            nn.ReLU(True),
        ...
        self.public_dense_p_layers = PSCN(1024, 512, 4)
        ...
        self.public_v = nn.Sequential(
            nn.Conv2d(139, 128, 3, 1, padding=(1, 0)),  # 128*4,7
            nn.ReLU(True),
        ...
        self.public_dense_v_layers = PSCN(512, 256, 3)
    ...
    def forward(self, input_data):
        """
        obs: batch*(23+29*4)*4*9, first 12 are feature channel, last 64 (32*2) are search channel
        """
        # obs = 139 * 4 * 9
        obs = input_data["observation"].type(torch.float32)
        mask = input_data["action_mask"].type(torch.float32)
        obs_dense = obs[:, : 23 * 4 * 9].view(-1, 828)
        obs_dense = torch.concat(
            [obs_dense[:, :18], obs_dense[:, 36:]], dim=1
        )  # batch * 810
        obs = obs.reshape(-1, 139, 4, 9)
...
if __name__ == "__main__":
    ...
    data = torch.rand(2, 139 * 4 * 9)
    mask = torch.ones(2, 235)
    net = MahJongCNNNet6_LargeV4("cpu")
