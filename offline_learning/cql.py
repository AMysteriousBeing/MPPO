...
class CQLPolicy(SACPolicy):
    ...
    def __init__(
        ...
        self.cql_log_alpha = torch.zeros(
    ...
    def calc_random_values(
        self, obs: torch.Tensor, random_act: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        ...
        if self._max_q_backup:
            with torch.no_grad():
                ...
                tmp_next_q1 = (
                    self.critic1_old(tmp_next_obss, tmp_next_actions)
                    .view(batch_size, self._num_repeat_actions, 1)
                    .max(1)[0]
                    .view(-1, 1)
                )
                tmp_next_q2 = (
                    self.critic2_old(tmp_next_obss, tmp_next_actions)
                    .view(batch_size, self._num_repeat_actions, 1)
                    .max(1)[0]
                    .view(-1, 1)
                )
        ...
        random_actions = (
            torch.FloatTensor(batch_size * self._num_repeat_actions, actions.shape[-1])
            .uniform_(self.action_space.low[0], self.action_space.high[0])
        ...
        next_obs_pi_value1, next_obs_pi_value2 = self.calc_pi_values(
            tmp_next_obss, tmp_obss
        )
        ...
        conservative_loss1 = (
            torch.logsumexp(cat_q1 / self._temperature, dim=1).mean()
            * self._cql_weight
            * self._temperature
            - q1.mean() * self._cql_weight
        )
        conservative_loss2 = (
            torch.logsumexp(cat_q2 / self._temperature, dim=1).mean()
            * self._cql_weight
            * self._temperature
        ...
        if self._with_lagrange:
            ...
            conservative_loss1 = cql_alpha * (
                conservative_loss1 - self._lagrange_threshold
            )
            conservative_loss2 = cql_alpha * (
                conservative_loss2 - self._lagrange_threshold
            )
