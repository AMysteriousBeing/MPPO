...
class FeatureAgent2Adapted2(MahjongGBAgent):
    ...
    # game1+unseen1+hand1+ meld4*4 + history_compact1 * 4 + (history29)*4
    normal_feature_space = (139, 4, 9)
    OFFSET_OBS_NORMAL_FEATRUE = {
        "GAME": 0,
        "UNSEEN": 1,
        "HAND": 2,
        "MELD": 3,
    ...
    def __init__(self, seatWind):
        ...
        self.history_compact = [[] for i in range(4)]
    ...
    def request2obs(self, request):
        ...
        if t[2] == "Play":
            ...
            self.history_compact[p].append(self.curTile)
    ...
    def _history_compact_embedding_update(self, p):
        play_history = self.history_compact[p]
        d = defaultdict(int)
        for tile in play_history:
            d[tile] += 1
        # update embedding
        for tile in d:
            self.obs[self.OFFSET_OBS["HISTORY_COMPACT"] + p, self.OFFSET_TILE[tile]] = (
                d[tile]
            )
    ...
    def feature_normal_from_normal(self, normal_obs):
        feature = np.zeros((self.normal_feature_space[0], 36), np.uint8)
        # game info 1
        feature[self.OFFSET_OBS_NORMAL_FEATRUE["GAME"], normal_obs[0]] = 4
        feature[self.OFFSET_OBS_NORMAL_FEATRUE["GAME"], normal_obs[1] + 4] = 4
        game_step_binary_list = list(
            int(i) * 4 for i in format(self.game_step, f"010b")
        )
        feature[self.OFFSET_OBS_NORMAL_FEATRUE["GAME"], 8:18] = game_step_binary_list

        # unseen 1
        for j in range(34):
            feature[self.OFFSET_OBS_NORMAL_FEATRUE["UNSEEN"], j] = normal_obs[j + 2]
        tile_list = []
        # hand 1
        for j in range(14):
            tile = normal_obs[36 + j]
            if tile != 255:
                tile_list.append(self.TILE_LIST[tile])
                feature[self.OFFSET_OBS_NORMAL_FEATRUE["HAND"], tile] += 1
        # meld 4*4
        for i in range(4):
            for j in range(4):
                offset = 89 + i * 45 + j * 4
                for k in range(4):
                    ...
                    if tile != 255:
                        feature[
                            self.OFFSET_OBS_NORMAL_FEATRUE["MELD"] + i * 4 + j, tile
                        ] += 1
        # detailed history 29*4
        for i in range(4):
            for j in range(29):
                action = normal_obs[60 + i * 45 + j]
                if action < self.OFFSET_ACT["Chi"]:
                    tile = action - self.OFFSET_ACT["Play"]
                    feature[
                        self.OFFSET_OBS_NORMAL_FEATRUE["HISTORY"] + i * 29 + j, tile
                    ] = 1
                elif action < self.OFFSET_ACT["Peng"]:
                    t = (action - self.OFFSET_ACT["Chi"]) // 3
                    color = "WTB"[t // 7]
                    num = t % 7 + 2
                    tile = "%s%d" % (color, num)
                    for k in range(-1, 2):
                        feature[
                            self.OFFSET_OBS_NORMAL_FEATRUE["HISTORY"] + i * 29 + j,
                            self.OFFSET_TILE[tile] + k,
                        ] = 1

                elif action < self.OFFSET_ACT["Gang"]:
                    tile = action - self.OFFSET_ACT["Peng"]
                    feature[
                        self.OFFSET_OBS_NORMAL_FEATRUE["HISTORY"] + i * 29 + j, tile
                    ] = 3
                elif action < self.OFFSET_ACT["AnGang"]:
                    tile = action - self.OFFSET_ACT["Gang"]
                    feature[
                        self.OFFSET_OBS_NORMAL_FEATRUE["HISTORY"] + i * 29 + j, tile
                    ] = 4
                elif action < self.OFFSET_ACT["BuGang"]:
                    tile = action - self.OFFSET_ACT["AnGang"]
                    feature[
                        self.OFFSET_OBS_NORMAL_FEATRUE["HISTORY"] + i * 29 + j, tile
                    ] = 4

                elif action != 255:
                    tile = action - self.OFFSET_ACT["BuGang"]
                    feature[
                        self.OFFSET_OBS_NORMAL_FEATRUE["HISTORY"] + i * 29 + j, tile
                    ] = 4
        # compact history 1*4
        for i in range(4):
            play_history = self.history_compact[i]
            d = defaultdict(int)
            for tile in play_history:
                d[tile] += 1
            # update embedding
            for tile in d:
                feature[
                    self.OFFSET_OBS_NORMAL_FEATRUE["HISTORY_COMPACT"] + i,
                    self.OFFSET_TILE[tile],
                ] = d[tile]

        feature = feature.reshape(np.prod(self.normal_feature_space))
        return feature
