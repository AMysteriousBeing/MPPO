...
def BJ_ddp_training_DQfD(
    ...
    if not config["load_ckpt"]:
        for i in tqdm(range(1000)):
            ...
            Q_s = model(states)
            margin = torch.ones((batch_size, 2)).to(rank)
            margin = margin / 2
    ...
    target_model = BlackJackNet5b().to(rank)
    ...
    while not learner_status_list[1]:
        ...
        demo_states = demo_obs

        # calculate supervised loss
        Q_s = model(demo_states)
        margin = torch.ones((demo_batch_size, 2)).to(rank)
        margin = margin / 2
        ...
        next_obs = torch.tensor(batch["next_state"]).to(rank)
        ...
        loss = dqn_loss + sv_loss * static_ratio  # - entropy_coeff * entropy_loss
