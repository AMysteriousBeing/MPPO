...
def Maze_ddp_training_PPOfD(
    ...
    while not learner_status_list[1]:
        ...
        for _ in range(config["epochs"]):
            ...
            demo_obs_cnn = (
                demo_obs[:, : 5 * 11 * 11].reshape(-1, 5, 11, 11).type(torch.float32)
            ).to(rank)
            demo_obs_dense = demo_obs[:, 5 * 11 * 11 :].type(torch.float32).to(rank)
            demo_mask = torch.ones(demo_batch_size, 4).to(rank)
            demo_actions = demo_batch["actions"].to(rank)
            demo_states = {
                "cnn_obs": demo_obs_cnn,
                "dense_obs": demo_obs_dense,
                "action_mask": demo_mask,
            }
