from model import MazeNet2, MazeGAILDiscriminator
import time
import torch
...
def Maze_ddp_training(
    ...
    if config["load_ckpt"]:
        ...
        if os.path.exists(ckpt_path):
            ...
            if config["load_from_sl"]:
                ...
                discriminator.load_state_dict(ckpt["discriminator_dict"])
    ...
    while not learner_status_list[1]:
        ...
        advs = batch["adv"]
        cnn_obs_np = batch["state"]["cnn_obs"][advs > 0.2]
        dense_obs_np = batch["state"]["dense_obs"][advs > 0.2]
        act_np = batch["action"][advs > 0.2]
        adv_np = batch["adv"][advs > 0.2]
        obs_np = np.concatenate(
            [cnn_obs_np.reshape(-1, 11 * 11 * 5), dense_obs_np], axis=1
        )

        cnn_obs = torch.tensor(batch["state"]["cnn_obs"]).to(rank)
        dense_obs = torch.tensor(batch["state"]["dense_obs"]).to(rank)
        mask = torch.tensor(batch["state"]["action_mask"]).to(rank)
        states = {"cnn_obs": cnn_obs, "dense_obs": dense_obs, "action_mask": mask}
        ...
        flattened_feature = torch.concat(
            [cnn_obs.view(-1, 11 * 11 * 5), dense_obs], dim=1
        )
        ...
        for _ in range(config["discriminator_step"] * gail_multiplier):
            ...
            logits_pi = discriminator(flattened_feature, actions)
        ...
        # Calculate rewards.
        with torch.no_grad():
            advs_gail = -F.logsigmoid(-discriminator(flattened_feature, actions))
        ...
        for _ in range(config["epochs"]):
            ...
            frame_value_loss += value_loss.item()
