from model import (
    MahJongCNNNet6_LargeV2,
    MahJongCNNNet6_LargeV4,
    MahJongCNNNet6_LargeV5,
    MahJongNet64a,
)
import time
import torch
...
def ddp_training_PPOfD(
    ...
    dataset = qlearning_dataset(config["path_to_data"], world_size, rank, True)
    # create buffer
    ...
    # create model
    model = MahJongCNNNet6_LargeV2(rank).to(rank)
    ...
    while not learner_status_list[1]:
        ...
        with torch.no_grad():
            states = {
                "observation": obs,
                "action_mask": mask,
                # "pos_tensor": positional_tensor_partial,
                # "pos_info_list": position_info_list,
            }
        ...
        for _ in range(config["epochs"]):
            ...
            if config["load_ckpt"] and config["load_from_sl"] and iterations < 1000:
                policy_loss *= 0
                entropy_loss *= 0
